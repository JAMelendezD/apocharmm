...
extern "C" apo_status
apo_restart_subscriber_as_subscriber(apo_subscriber **out,
                                     apo_restart_subscriber *subscriber) {
  const char *function_name = "apo_restart_subscriber_as_subscriber";
  ...
  return apocharmm_c::guard(
      [&](void) -> apo_status {
        ...
        APOCHARMM_C_RETURN_IF_ERROR(
            apocharmm_c::require_handle_object<apo_restart_subscriber>(
                subscriber, function_name, "RestartSubscriber"));
